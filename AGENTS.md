...
- **Deploy-coupling / count-refresh caveat (manual & review):** the committed
  ...
  the manual/review HTML reflect the last-deployed snapshot. **Worked example (2026-06-21):** the H-4 deploy (PRs #135 payload+counts, #136 re-prove) advanced the snapshot to pytest **478** / frontend **70** / Python mutation **123** at `dataHash 6c18e572…` (public hosting `proven`/`matched`) — the step-by-step follows the deploy-coupling & count-refresh policy in `docs/{MANUAL,REVIEW}_GENERATION_GUIDE.md`.

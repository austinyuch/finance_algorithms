...
| 10 | **測試假綠** | mutation spot checks kill representative domain/governance mutations; current configured suite is 41/41 after adding B source-quorum proof exit/file gating and consolidating exact PR-pair memo checks into a non-self-staling promotion-boundary guard, including root Torch dependency, stale governance evidence, exhaustive PR-ledger regression, public-hosting manifest/probe drift, scheduled-run observer guards, scoped source-health overclaim, broad source-quorum overclaim, source-quorum proof replay overclaim, E Tier3 readiness overclaim, E serving smoke health gating, E retraining smoke status gating, E production-tier evidence gating, E automated drift status gating, E production serving endpoint gating, E production retraining status gating, and E readiness CLI validator bypass | `scripts/run_mutation_spot_checks.py`; `quantlab/TESTS.md` |

驗證指令:`uv run pytest -q`(231 passed)· `uv run mypy quantlab/ scripts/run_tsmc_hedge_slice.py scripts/scheduled_run_observer.py scripts/tier3_readiness_gate.py scripts/source_quorum_proof.py --ignore-missing-imports`(clean)· `uv run lint-imports`(KEPT)· `uv run pytest --cov=quantlab.mlops.experiment_registry --cov-report=term-missing tests/quantlab/test_e_1_experiment_registry.py`(100% line coverage)· `uv run python scripts/run_mutation_spot_checks.py`(41/41 killed)

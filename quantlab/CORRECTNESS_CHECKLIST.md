...
| 10 | **測試假綠** | mutation spot checks kill representative domain/governance mutations; current configured suite is 27/27, including root Torch dependency, stale governance evidence, scheduled-run observer guards, and E Tier3 readiness overclaim | `scripts/run_mutation_spot_checks.py`; `quantlab/TESTS.md` |

驗證指令:`uv run pytest -q`(196 passed, 1 skipped; PyTorch LSTM optional lane skipped in default env)· `uv run mypy quantlab/ scripts/run_tsmc_hedge_slice.py scripts/scheduled_run_observer.py --ignore-missing-imports`(clean)· `uv run lint-imports`(KEPT)· `uv run python scripts/run_mutation_spot_checks.py --only e-tier3-readiness-gate`(killed; full configured suite now 27/27)

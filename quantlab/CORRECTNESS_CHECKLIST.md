...
| 10 | **測試假綠** | mutation spot checks kill representative domain/governance mutations; current configured suite is 72/72 after adding A0 event-date replay gating, B source-quorum proof exit/file gating, Stooq proof exit/file gating, public-hosting content-hash/contract/probe-status/review-probe-status/probe-hash/taxonomy-authority proof gating, stakeholder and app payload sync drift gating, review pytest/frontend-count/audit transcript freshness gating, failed frontend transcript gating, import-linter count/formalization drift gating, E evidence explicit claim-boundary gating, stale visual-count regression gating, and consolidating exact PR-pair memo checks into a non-self-staling promotion-boundary guard, including root Torch dependency, stale governance evidence, exhaustive PR-ledger regression, stale dashboard source wording, stale visual evidence wording, public-hosting manifest/probe/review-probe/hash/contract/taxonomy drift, scheduled-run observer guards, scoped source-health overclaim, broad source-quorum overclaim, source-quorum proof replay overclaim, Stooq proof replay/exit overclaim, E Tier3 readiness overclaim, E readiness gate proof-digest drift, E readiness gate production-validator bypass, E serving smoke health gating, E retraining smoke status gating, E production-tier evidence gating, E automated drift status gating, E production serving endpoint gating, E production retraining status gating, and E readiness CLI validator bypass | `scripts/run_mutation_spot_checks.py`; `quantlab/TESTS.md` |

驗證指令:`uv run pytest -q`(256 passed)· `uv run mypy quantlab/ scripts/build_showcase_payload.py scripts/run_tsmc_hedge_slice.py scripts/scheduled_run_observer.py scripts/tier3_readiness_gate.py scripts/source_quorum_proof.py scripts/stooq_contract_proof.py --ignore-missing-imports`(clean)· `uv run lint-imports`(KEPT)· `uv run pytest --cov=stooq_contract_proof --cov=quantlab.data.source_health --cov-report=term-missing tests/test_daily_snapshot.py tests/quantlab/test_b_6_source_health.py -k 'stooq_contract_proof or stooq_reopen_evidence or stooq_source_contract_decision or source_health'`(90% line coverage)· `uv run python scripts/run_mutation_spot_checks.py`(72/72 killed)

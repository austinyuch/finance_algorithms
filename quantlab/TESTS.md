...
- F Next.js: `cd frontend && npm run coverage` → 92.13% line coverage; `npm audit --json` → 0 vulnerabilities; `npm run mutation` → 8/8 killed; `npm run visual`, `npm run visual:browser`, `npm run probe:public-demo`, `npm run build`, and `npm run smoke` on `127.0.0.1:3044` passed.

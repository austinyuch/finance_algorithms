...
Spec coverage: A0, A with optional PyTorch LSTM lane isolated from default runtime, B including broad source-quorum gating, C, D model families/evaluation/artifacts, E-lite registry durability/Tier3 artifact manifest/drift assessment/local serving/retraining/automated drift monitoring smoke evidence with production-tier readiness gating, governed production evidence probes, and strict readiness proof CLI, F showcase/demo hardening/public static showcase with hosted proof and visual diff, G alt-data bundle loading, and legacy `invest_algorithms` regression.
Canonical command: `uv run pytest -q`
Last refreshed: 2026-06-12 · Python full suite **227 passed** (default env; PyTorch LSTM optional-lane no longer appears in the collected suite; B source-quorum gate added; exact PR-pair governance checks consolidated into a non-self-staling promotion-boundary guard) · `uv run mypy quantlab/ scripts/run_tsmc_hedge_slice.py scripts/scheduled_run_observer.py scripts/tier3_readiness_gate.py --ignore-missing-imports` clean over 53 files · `uv run lint-imports` KEPT · `scripts.run_mutation_spot_checks` focused line coverage 90% · Python mutation spot checks: 39/39 configured/killed after adding `b-source-quorum-status-gate`; latest `b-source-quorum-status-gate` killed · E registry focused line coverage 100% · root `uv.lock` excludes Torch · F Next.js tests 23 passed, coverage 91.42%, npm audit 0 vulnerabilities, 9/9 frontend mutations killed, visual export/browser visual pixel diff/build/smoke/public probe passed
...
| `test_daily_snapshot` | bitemporal stamping, append-only write, scoped source selection, caller-provided output root, scoped source-health summary, parser wrappers, Yahoo latest-close PBT, Stooq opt-in policy, graceful degradation, machine-readable run report, B snapshot ops gate, CR-B18 source-quorum gate, schedule report/proof retention, schedule workflow contract, workflow timestamp guard, scheduled run observer, Stooq decision/reopen evidence helper | data vintage routine / CR-B8 / CR-B9 / CR-B11 / CR-B12 / CR-B18 / b-snapshot-ops-gate / next-gaps-1-6-tier3-public / ops-visual-drift-artifacts / b-live-scheduled-snapshot-proof / b-scheduled-run-observer | 33 pass; live `event=schedule` run `27392471359` proven; CR-B12 local live write smoke `ok=1` then `skip=1`; CR-B18 unit/CLI/PBT/chaos source-quorum gate rejects scoped/dry/failed broad-readiness reports |
...
| `test_governance_guards` | import-linter wrapper, spec/runtime interface drift guard, current governance stale-evidence guard, post-merge scheduled-observer resume guard, post-merge governance-sync guard, post-merge E gate promotion guard, cron-proof stale-state guard, non-self-staling promotion-boundary guard, and CR-FPS-001 public-hosting manifest/probe proof guard for `NEXT_STEPS.md` / `ISSUE_LOG.md` / correctness checklist / `docs/deployment-manifest.json` | residual hardening + governance-evidence-refresh / b-scheduled-run-observer / e-tier3-readiness-gate / b-data-platform CR-B12..CR-B17 / f-public-static-showcase CR-FPS-001 | 9 pass |
...
- F Next.js: `cd frontend && npm run coverage` → 91.42% line coverage; `npm audit --json` → 0 vulnerabilities; `npm run mutation` → 9/9 killed; `npm run visual`, `npm run visual:browser` (repo-baseline pixel diff `234 / 1,296,000`, threshold `0.001`), `npm run probe:public-demo`, `npm run build`, and `npm run smoke` on `127.0.0.1:3044` passed.

...
MUTATIONS: tuple[MutationSpec, ...] = (
    MutationSpec(
        ...
        path="quantlab/research/model_performance_report.py",
        ...
        test_command=("uv", "run", "pytest", "-q",
                      "tests/quantlab/test_h_model_performance_report.py::test_report_ranks_oos_net_only_and_keeps_baseline_visible"),
    ),
    MutationSpec(
        name="h-deep-report-no-alpha-claim",
        path="quantlab/research/model_performance_report.py",
        original='_CLAIM = "no_alpha_claim"',
        mutated='_CLAIM = "alpha_claim"',
        test_command=("uv", "run", "pytest", "-q",
                      "tests/quantlab/test_h_model_performance_report.py::test_report_ranks_oos_net_only_and_keeps_baseline_visible"),
    ),
    MutationSpec(
        name="h-backend-honest-fallback",
        path="quantlab/models/dl/backends.py",
        original="            name=_REFERENCE,\n            requested=requested,\n            available=True,",
        mutated="            name=_REFERENCE,\n            requested=requested,\n            available=False,",
        test_command=("uv", "run", "pytest", "-q",
    ...
    MutationSpec(
        name="real-data-oos-sufficiency-asset-gate",
        path="quantlab/research/real_data_oos.py",
        original="if len(assets) < min_assets:",
        mutated="if False and len(assets) < min_assets:",
        test_command=("uv", "run", "pytest", "-q",
                      "tests/quantlab/test_real_data_oos.py::test_sufficiency_single_asset_fails_closed"),
    ),
    MutationSpec(
        name="real-data-oos-net-sharpe-segment",
        path="quantlab/research/real_data_oos.py",
        original='if metric.get("segment") == "out_of_sample" and metric.get("basis") == "net":',
        mutated='if metric.get("segment") == "in_sample" and metric.get("basis") == "net":',
        test_command=("uv", "run", "pytest", "-q",
                      "tests/quantlab/test_real_data_oos.py::test_report_oos_net_sharpe_uses_out_of_sample_not_in_sample"),
    ),
    MutationSpec(
        name="real-data-oos-baseline-visibility",
        path="quantlab/research/real_data_oos.py",
        original="if not any(bool(r.get(\"is_baseline\")) for r in rows):",
        mutated="if False and not any(bool(r.get(\"is_baseline\")) for r in rows):",
        test_command=("uv", "run", "pytest", "-q",
    ...
    MutationSpec(
        ...
        path="quantlab/showcase/scenario.py",
        original="evidence_tests=_current_evidence_tests(evidence_root, asof=asof),",
    ...
    MutationSpec(
        ...
        test_command=("uv", "run", "pytest", "-q",
                      "tests/quantlab/test_f_1_showcase_api.py::test_hosting_freshness_downgrades_deterministically"),
    ),
    MutationSpec(
        name="f-showcase-public-probe-observed-at-gate",
        path="quantlab/showcase/scenario.py",
        original="if observed_at_dt > asof:",
        mutated="if False and observed_at_dt > asof:",
        test_command=("uv", "run", "pytest", "-q",
                      "tests/quantlab/test_f_1_showcase_api.py::test_canonical_showcase_artifact_rejects_failed_browser_visual_evidence"),
    ),
    MutationSpec(
        ...
        path="quantlab/showcase/scenario.py",
        original="if observed_at_dt + timedelta(hours=max_age_hours) <= asof:",
        mutated="if False and observed_at_dt + timedelta(hours=max_age_hours) <= asof:",
        test_command=("uv", "run", "pytest", "-q",
                      "tests/quantlab/test_f_1_showcase_api.py::test_hosting_freshness_downgrades_deterministically"),
    ),
    MutationSpec(
    ...
    *_public_hosting_proof_mutations(),
    MutationSpec(
        ...
        path="quantlab/showcase/scenario.py",
        original="if observed_at_dt + timedelta(hours=max_age_hours) <= asof:",
        mutated="if observed_at_dt + timedelta(hours=max_age_hours) < asof:",
        test_command=("uv", "run", "pytest", "-q",
                      "tests/quantlab/test_f_1_showcase_api.py::test_hosting_freshness_window_boundary_is_inclusive"),
    ),
    MutationSpec(
        name="public-hosting-stale-downgrade-not-proven",
        path="quantlab/showcase/scenario.py",
        original='if freshness == "stale":',
        mutated='if freshness == "fresh":',
        test_command=("uv", "run", "pytest", "-q",
                      "tests/quantlab/test_f_1_showcase_api.py::test_hosting_freshness_downgrades_deterministically"),
    ),
    MutationSpec(

...
- `uv run pytest -q` → **190 passed, 1 skipped** (PyTorch LSTM optional-lane tests skipped in default env)
...
- Python mutation spot checks → **24/24 configured**, including `root-torch-default-dependency` and `governance-stale-next-steps-alert`; frontend coverage **91.42%**, mutation 9/9 killed

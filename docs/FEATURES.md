...
- `uv run pytest -q` → **227 passed** (default env)
...
- Python mutation spot checks → **44/44 configured/killed**, including `snapshot-scoped-source-health`, `root-torch-default-dependency`, `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, `governance-stale-cron-proof-pending`, `governance-stale-cr-b14-promotion`, `governance-stale-cr-b15-promotion`, `governance-stale-cr-b16-promotion`, `governance-stale-cr-b17-promotion`, `governance-stale-cr-fps001-promotion`, `public-hosting-manifest-status-regression`, `e-tier3-readiness-gate`, `e-serving-smoke-health-gate`, `e-retraining-smoke-status-gate`, `e-tier3-production-tier-gate`, `e-automated-drift-status-gate`, `e-production-serving-endpoint-gate`, `e-production-retraining-status-gate`, `e-tier3-cli-serving-validator`, and `b-scheduled-observer-manual-pending`; frontend coverage **91.42%**, mutation 9/9 killed

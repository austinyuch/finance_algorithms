...
| 6 | **MLOps E-lite** (experiment registry, config catalog, checksum snapshot durability, registry→dashboard bridge, Tier3 readiness gate, local serving and retraining smoke evidence) | E | library / read API | PASSED | `test_e_1`, registry JSONL, readiness gate, serving/retraining smoke evidence | Local smoke only; gate blocks Tier3 ready without automated drift evidence |
...
- `uv run pytest -q` → **204 passed, 1 skipped** (PyTorch LSTM optional-lane tests skipped in default env)

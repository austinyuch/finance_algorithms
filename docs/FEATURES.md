...
- `uv run pytest -q` → **193 passed, 1 skipped** (PyTorch LSTM optional-lane tests skipped in default env)
...
- Python mutation spot checks → **26/26 configured**, including `root-torch-default-dependency`, `governance-stale-next-steps-alert`, `governance-stale-scheduled-observer-promotion`, and `b-scheduled-observer-manual-pending`; frontend coverage **91.42%**, mutation 9/9 killed

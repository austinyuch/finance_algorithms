...
uv run pytest -q             # sanity: expect 226 passed
...
> - Resolved: visual diff is repo-baseline pixel-backed (`230 / 1,296,000`
...
- Test suite is now **226 passed** after moving PyTorch LSTM proof to the optional lane and adding current-governance stale-evidence guards; mypy is clean over **53** files; mutation spot checks are **43/43 configured/killed**, including root Torch dependency, stale governance evidence mutations through CR-B17 promotion drift, CR-FPS-001 public-hosting manifest/probe drift, and CR-B12 scoped source-health overclaim protection.
...
- **Visual diff now repo-baseline pixel-backed** (`browser-visual-diff.json`: `230 / 1,296,000` mismatched pixels at threshold `0.001`) — closes the prior hash-equality residual while allowing small text-rendering drift under the gate.

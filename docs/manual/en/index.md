...
uv run pytest -q             # sanity: expect 240 passed
...
> - Resolved: visual diff is repo-baseline pixel-backed (`966 / 1,296,000`
...
- **Visual diff now repo-baseline pixel-backed** (`browser-visual-diff.json`: `966 / 1,296,000` mismatched pixels at threshold `0.001`) — closes the prior hash-equality residual while allowing small text-rendering drift under the gate.

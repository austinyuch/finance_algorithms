...
> - Coverage Tier: `hybrid` · Readiness State: `CONDITIONAL` (`f-demo-hardening/review.md`); browser visual + public-hosting probe `PASSED` with content-hash parity (`ops-visual-drift-artifacts/review.md`)
...
>   probe is `proven` with HTTP 200 plus deployed dataHash parity, but the export's embedded readiness panel
...
- Test suite is now **236 passed** after moving PyTorch LSTM proof to the optional lane and adding current-governance stale-evidence guards; mypy is clean over **55** files; mutation spot checks are **44/44 configured/killed**, including root Torch dependency, stale governance evidence mutations plus the non-self-staling promotion-boundary guard, CR-FPS-001/CR-FPS-002 public-hosting manifest/probe/hash drift, CR-B12 scoped source-health overclaim protection, CR-B18 broad source-quorum overclaim protection, CR-B19 proof replay protection, and CR-B20 Stooq proof exit/file replay protection.
...
- **Public-hosting probe now proven** HTTP 200 plus deployed dataHash parity (`public-hosting-probe.json`) — closes the prior `configured_not_observed` gap.

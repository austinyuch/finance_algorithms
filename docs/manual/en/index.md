...
>   probe is `proven` with HTTP 200 plus deployed dataHash and manifest-contract parity, but the export's embedded readiness panel
...
- **Public-hosting probe now proven** HTTP 200 plus deployed dataHash and manifest-contract parity (`public-hosting-probe.json`) — closes the prior `configured_not_observed` gap.

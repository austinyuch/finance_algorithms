...
uv run pytest -q             # sanity: expect 221 passed
...
- Test suite is now **221 passed** after moving PyTorch LSTM proof to the optional lane and adding current-governance stale-evidence guards; mypy is clean over **53** files; mutation spot checks are **37/37 configured/killed**, including root Torch dependency, stale governance evidence mutations, and CR-B12 scoped source-health overclaim protection.

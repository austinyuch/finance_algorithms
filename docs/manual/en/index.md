...
> - Coverage Tier: `hybrid` · Readiness State: `CONDITIONAL` (`f-demo-hardening/review.md`); browser visual `PASSED`; public hosting is `proven` / `matched` for deployed==expected `dataHash 6c18e572…` after the 2026-06-21 `main` deploy (live-probed 2026-06-21T04:18Z via `scripts/refresh_public_hosting_proof.py --live`); the dashboard self-claim stays `not_proven` by static-artifact contract (`docs/public-hosting-probe.json`)
...
>   observes HTTP 200 plus manifest-contract metadata and a matched deployed hash
>   (`status=proven`, deployed==expected `dataHash 6c18e5720c69ce322388e96a60097cd17b79b891b2cb824071f3cb542f879279…` — live-probed 2026-06-21T04:18Z after the 2026-06-21 `main` deploy via `scripts/refresh_public_hosting_proof.py --live`); freshness is
...
>   by static-artifact contract — a static artifact cannot self-claim its deployment; the `proven`
...
>   killed. Public hosting is `proven` / `matched` for deployed==expected
>   `dataHash 6c18e572…` after the 2026-06-21 `main` deploy (live-probed
>   2026-06-21T04:18Z via `scripts/refresh_public_hosting_proof.py --live`); the
>   dashboard self-claim stays `not_proven` by static-artifact contract.
...
- **Public hosting is proven after the 2026-06-21 `main` deploy.**
  Public hosting is `proven` / `matched` for deployed==expected `dataHash 6c18e5720c69ce322388e96a60097cd17b79b891b2cb824071f3cb542f879279…`
  (`docs/deployment-manifest.json`,
  `docs/public-hosting-probe.json`, observed `2026-06-21T04:18Z`) — live-probed
  via `scripts/refresh_public_hosting_proof.py --live` after the 2026-06-21 `main`
  deploy. Older
  `e5794260…` / `0f170441…` hosting proofs remain historical point-in-time evidence
  only. The dashboard self-claim stays `not_proven` by static-artifact contract.
...
- **Public-hosting probe records HTTP 200 and deployed manifest contract metadata** (`public-hosting-probe.json`); after each dashboard `dataHash` refresh, branch-local parity is `configured_not_observed` until Pages serves it, then re-proven on deploy — currently **`proven`** / `matched` for deployed==expected `dataHash 6c18e572…` after the 2026-06-21 `main` deploy (live-probed 2026-06-21T04:18Z via `scripts/refresh_public_hosting_proof.py --live`).
...
| Dashboard payload `publicHosting` self-claim stays `not_proven` **by contract** (a static artifact can't self-claim deployment); external committed probe/manifest parity for `dataHash 6c18e572…` is `proven` / `matched` (deployed==expected) after the 2026-06-21 `main` deploy (live-probed 2026-06-21T04:18Z) | by design | `frontend/out/index.html`, `docs/public-hosting-probe.json` |

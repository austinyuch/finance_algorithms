...
uv run pytest -q             # sanity: expect 433 passed, 2 skipped
...
> - Coverage Tier: `hybrid` · Readiness State: `CONDITIONAL` (`f-demo-hardening/review.md`); browser visual `PASSED`; current branch-local public hosting is `configured_not_observed` at expected `dataHash dc7bb732…` until Pages serves the refreshed artifact (`docs/public-hosting-probe.json`)
...
>   (`status=configured_not_observed`, expected `dataHash dc7bb732…`); freshness is
...
- **Public hosting remains deploy-gated for the refreshed artifact.**
  The current committed manifest expects `dataHash dc7bb732…`, while the live
  ...
  `2026-06-18T03:10Z`). Older CR-B21 hosting proof at `0f170441…` remains
...
| Dashboard payload `publicHosting` self-claim stays `not_proven` by contract; current committed probe/manifest parity is `configured_not_observed` until Pages serves `dataHash dc7bb732…` | Low | `frontend/out/index.html`, `docs/public-hosting-probe.json` |

...
> - Coverage Tier: `hybrid` · Readiness State: `CONDITIONAL`（`f-demo-hardening/review.md`）；browser visual `PASSED`，public-hosting probe 已觀測 HTTP 200、deployed manifest contract metadata 與匹配的 deployed `dataHash`（`ops-visual-drift-artifacts/review.md`）
...
>   probe 已觀測 HTTP 200、deployed manifest contract metadata 與匹配的 deployed `dataHash`；export 內嵌 readiness 面板依 dashboard contract 仍保守顯示 `not_proven`。
...
- **Public-hosting probe 已記錄 HTTP 200、deployed manifest contract metadata 與匹配的 deployed `dataHash`**（`public-hosting-probe.json`）；export readiness 面板仍維持 `not_proven`，因 dashboard 是 fixture-backed local demo evidence。

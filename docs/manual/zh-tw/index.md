...
> - Coverage Tier: `hybrid` · Readiness State: `CONDITIONAL`（`f-demo-hardening/review.md`）；browser visual `PASSED`；已提交 payload（`dataHash 6c18e572…`）的 public hosting 為 `configured_not_observed` / `mismatched`（待部署）——已於 2026-06-20 對 Pages live-probe，線上仍服務先前的 `c33da57d…`；待本次合併且 `main`→Pages 重新部署後會重新證明為 `proven` / `matched`（`docs/public-hosting-probe.json`）
...
>   與 manifest-contract metadata，但 deployed hash 不相符
>   （`status=configured_not_observed`、expected `dataHash 6c18e5720c69ce322388e96a60097cd17b79b891b2cb824071f3cb542f879279…`、deployed 仍為先前的 `dataHash c33da57d11c48945abcee36f2c78eb377f793536f769ddb10b87e8e4b3c7462a…` — 已於 2026-06-20 live-probe，待 `main`→Pages 重新部署）；freshness 現為 deterministic，過期證據會降級
...
>   皆 killed。已提交 payload（`dataHash 6c18e572…`）的 public hosting 為
>   `configured_not_observed` / `mismatched`（待部署）——已於 2026-06-20 對 Pages
>   live-probe，線上仍服務先前的 `c33da57d…`；待本次合併且 `main`→Pages 重新部署後
>   會重新證明為 `proven` / `matched`；儀表板 self-claim 依設計維持 `not_proven`。
...
- **Public hosting 在最新 payload 重生後為待部署。**
  已提交 payload（`dataHash 6c18e5720c69ce322388e96a60097cd17b79b891b2cb824071f3cb542f879279…`）的
  public hosting 為 `configured_not_observed` / `mismatched`（`docs/deployment-manifest.json`、
  `docs/public-hosting-probe.json`，觀測於 `2026-06-20T16:58Z`）——已對 Pages live-probe，
  線上仍服務先前的 `dataHash c33da57d11c48945abcee36f2c78eb377f793536f769ddb10b87e8e4b3c7462a…`；
  待本次合併且 `main`→Pages 重新部署後，會以
  `scripts/refresh_public_hosting_proof.py --live` 重新證明為 `proven` / `matched`。較舊的
...
- **Public-hosting probe 已記錄 HTTP 200 與 deployed manifest contract metadata**（`public-hosting-probe.json`）；每次 dashboard `dataHash` 刷新後 branch-local parity 為 `configured_not_observed`，直到 Pages 服務後於部署時重新證明 — 目前對已提交 `dataHash 6c18e572…` 為 **`configured_not_observed`** / `mismatched`（待部署），線上仍服務先前的 `c33da57d…`（已於 2026-06-20 live-probe）。
...
| Dashboard payload `publicHosting` self-claim 依契約維持 `not_proven`（靜態 artifact 無法自我宣稱部署）；外部 committed probe/manifest parity 對已提交 `dataHash 6c18e572…` 為 `configured_not_observed` / `mismatched`（待部署，線上仍服務先前的 `c33da57d…`），待 `main`→Pages 重新部署後重新證明為 `proven` / `matched` | by design | `frontend/out/index.html`、`docs/public-hosting-probe.json` |

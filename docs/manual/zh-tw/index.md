...
uv run pytest -q             # 健檢：預期 246 passed
...
> - 已解決：visual diff 為 repo-baseline pixel-backed（`1036 / 1,296,000`
...
- **Visual diff 已改為 repo-baseline pixel-backed**（`browser-visual-diff.json`：`1036 / 1,296,000` mismatched pixels，threshold `0.001`）— 解決先前 hash-equality 殘留，同時允許 gate 內的少量文字渲染差異。

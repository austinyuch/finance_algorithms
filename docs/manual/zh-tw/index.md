...
> - Coverage Tier: `hybrid` · Readiness State: `CONDITIONAL`（`f-demo-hardening/review.md`）；browser visual `PASSED`；public hosting 為 `proven` / `matched`，deployed==expected `dataHash 6c18e572…`，在 2026-06-21 `main` 部署之後（已於 2026-06-21T04:18Z 以 `scripts/refresh_public_hosting_proof.py --live` live-probe）；儀表板 self-claim 依 static-artifact 契約維持 `not_proven`（`docs/public-hosting-probe.json`）
...
>   （`status=proven`、deployed==expected `dataHash 6c18e5720c69ce322388e96a60097cd17b79b891b2cb824071f3cb542f879279…` — 已於 2026-06-21T04:18Z 在 2026-06-21 `main` 部署之後以 `scripts/refresh_public_hosting_proof.py --live` live-probe）；freshness 現為 deterministic，過期證據會降級
>   而非 crash（CR-FPS-011）。Dashboard payload 自身的 `publicHosting` self-claim
>   依 static-artifact 契約**維持 `not_proven`** — static artifact 不能自我宣稱其部署；`proven` 狀態
...
>   皆 killed。public hosting 為 `proven` / `matched`，deployed==expected
>   `dataHash 6c18e572…`，在 2026-06-21 `main` 部署之後（已於 2026-06-21T04:18Z
>   以 `scripts/refresh_public_hosting_proof.py --live` live-probe）；儀表板
>   self-claim 依 static-artifact 契約維持 `not_proven`。
...
- **Public hosting 在 2026-06-21 `main` 部署之後為 proven。**
  public hosting 為 `proven` / `matched`，deployed==expected `dataHash 6c18e5720c69ce322388e96a60097cd17b79b891b2cb824071f3cb542f879279…`
  （`docs/deployment-manifest.json`、
  `docs/public-hosting-probe.json`，觀測於 `2026-06-21T04:18Z`）——在 2026-06-21 `main` 部署之後
  以 `scripts/refresh_public_hosting_proof.py --live` live-probe。較舊的
  `e5794260…` / `0f170441…` hosting proof 只保留為 historical point-in-time evidence；
  儀表板 self-claim 依 static-artifact 契約維持 `not_proven`。
...
- **Public-hosting probe 已記錄 HTTP 200 與 deployed manifest contract metadata**（`public-hosting-probe.json`）；每次 dashboard `dataHash` 刷新後 branch-local parity 為 `configured_not_observed`，直到 Pages 服務後於部署時重新證明 — 目前對 `dataHash 6c18e572…` 為 **`proven`** / `matched`（deployed==expected），在 2026-06-21 `main` 部署之後（已於 2026-06-21T04:18Z 以 `scripts/refresh_public_hosting_proof.py --live` live-probe）。
...
| Dashboard payload `publicHosting` self-claim 依契約維持 `not_proven`（靜態 artifact 無法自我宣稱部署）；外部 committed probe/manifest parity 對 `dataHash 6c18e572…` 為 `proven` / `matched`（deployed==expected），在 2026-06-21 `main` 部署之後（已於 2026-06-21T04:18Z live-probe） | by design | `frontend/out/index.html`、`docs/public-hosting-probe.json` |

...
uv run pytest -q             # 健檢：預期 224 passed
...
> - 已解決：visual diff 為 repo-baseline pixel-backed（`235 / 1,296,000`
...
- 測試套件目前為 **224 passed**（PyTorch LSTM proof 移至 optional lane，並新增 current-governance stale-evidence guards）；mypy 現涵蓋 **53** 檔且 clean；mutation spot checks **41/41 configured/killed**，包含 root Torch dependency、延伸到 CR-B16 promotion drift 的 stale governance evidence mutations 與 CR-B12 scoped source-health overclaim 防護。
...
- **Visual diff 已改為 repo-baseline pixel-backed**（`browser-visual-diff.json`：`235 / 1,296,000` mismatched pixels，threshold `0.001`）— 解決先前 hash-equality 殘留，同時允許 gate 內的少量文字渲染差異。

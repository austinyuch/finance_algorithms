...
## Showcase dashboard UI (shadcn/Tailwind) — source note

The deployed showcase dashboard (`docs/index.html`, served on Pages) is generated by
`frontend/scripts/export-public-demo.tsx` and styled with **shadcn-style components + Tailwind v4**
(`frontend/components/ui/*`, `frontend/components/Dashboard.tsx`). The export compiles Tailwind
(`npm run build:export-css`) and **inlines the CSS into the export `<head>`** so the deployed file is
self-contained (no CDN/external link). Re-pinning the dashboard's visual is deploy-coupled
(static visual contract `frontend/visual-baselines/showcase.visual.json` + browser VRT baseline).
The dashboard's `dataHash` is `sha256(JSON.stringify(dashboard data))` — a UI-only restyle does **not**
change it, so a pure styling change keeps public hosting `proven` without a live re-probe.
Spec: `.agents/specs/h-showcase-shadcn-ui/`. `no_alpha_claim` throughout.

...
| [b-data-platform](./b-data-platform/) | B | **Implemented(repo-side)· Review PASSED**(B-1/2/4/5;B-3 bulk=真機 handoff;CR-B7/B8/B9/B10 source policy/status) | a0-backtest-foundation | a0(history()/metrics additive;**CR-B5** pit_strictness additive schema); snapshot source defaults/status(CR-B7/CR-B8/CR-B9/CR-B10) | — (CR-B5/CR-B7/CR-B8/CR-B9/CR-B10 Implemented);residual `ISSUE-B3-001` for Stooq availability only | `spec/b-data-platform`; `spec/e-lite-source-f-hardening` |
...
| [_e-mlops-tier3_](./e-mlops-tier3-readiness.md) + [e-mlops-tier3-lite](./e-mlops-tier3-lite/) | E | **Implemented(E-lite registry) · Review PASSED**(registry-only; no serving/retraining/drift yet) | d(3 model families),f-showcase-read-api-dashboard | experiment registry / config catalog / future dashboard read API | — | `spec/e-lite-source-f-hardening` |
| [f-showcase-read-api-dashboard](./f-showcase-read-api-dashboard/) + [f-nextjs-showcase-dashboard](./f-nextjs-showcase-dashboard/) + [f-demo-hardening](./f-demo-hardening/) | F | **Implemented(local Next.js runtime + demo honesty hardening)· Review PASSED**(local HTTP smoke; public hosting/visual regression deferred) | a0(read API),c-portfolio-core,d-first-regime-model,d-return-risk-forecast-model,d-robust-portfolio-optimization-model,e-mlops-tier3-lite | QuantLab result-store/read surface; no legacy API behavior change | — | `spec/f-showcase-and-d-return-risk`; `spec/e-lite-source-f-hardening` |

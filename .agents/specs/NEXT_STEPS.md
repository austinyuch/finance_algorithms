...
- **Current branch lane:** `spec/f-showcase-real-payload` (CR-FPS-006 canonical dashboard source overlay plus CR-FPS-007 public probe parity-status hardening against completed F baseline).
...
- **Latest evidence:** `uv run pytest -q` → **239 passed** (default env, after adding the B source-quorum live proof wrapper, Stooq contract proof wrapper, CR-FPS-006 canonical dashboard payload source, and consolidating exact PR-pair memo guards into a non-self-staling promotion-boundary guard); `uv run mypy quantlab/ scripts/build_showcase_payload.py scripts/run_tsmc_hedge_slice.py scripts/scheduled_run_observer.py scripts/tier3_readiness_gate.py scripts/source_quorum_proof.py scripts/stooq_contract_proof.py --ignore-missing-imports` → clean over 57 files; `uv run lint-imports` → KEPT over 74 files / 185 dependencies; Stooq proof/source-health focused coverage → 11 passed, 90% combined line coverage; Python mutation spot checks **48/48 configured/killed**, including `b-source-quorum-status-gate`, `b-source-quorum-proof-exit-gate`, `b-source-quorum-proof-file-gate`, `b-stooq-proof-exit-gate`, `b-stooq-proof-file-gate`, `governance-exhaustive-pr-ledger-regression`, `public-hosting-manifest-status-overclaim`, `public-hosting-probe-status-overclaim`, `public-hosting-manifest-hash-overclaim`, `public-hosting-manifest-contract-regression`, and `public-hosting-taxonomy-authority-regression`; default `uv sync` removes `torch`/CUDA packages from the root env; F Next.js tests 28 passed, line coverage 91.07%, `npm audit --json` 0 vulnerabilities, 13/13 frontend mutations killed, static export/browser visual pixel diff/build/smoke passed, and public probe now fail-closed with `status=configured_not_observed` / exit 2 while deployed hash is stale.
...
- **F public static showcase:** [f-public-static-showcase](./f-public-static-showcase/) is **Implemented · Review PASSED**; GitHub Pages static `docs/` artifact path is configured, `frontend/out` export is reproducible, visual contract baseline is pinned, [CR-FPS-001](./f-public-static-showcase/change-requests/cr-fps-001-hosting-manifest-proof-sync.md) keeps the committed `docs/deployment-manifest.json` aligned with tracked HTTP 200 public-hosting proof, [CR-FPS-002](./f-public-static-showcase/change-requests/cr-fps-002-hosting-content-hash-proof.md) requires deployed `dataHash` parity, [CR-FPS-003](./f-public-static-showcase/change-requests/cr-fps-003-hosting-manifest-contract-proof.md) requires manifest-contract parity, [CR-FPS-004](./f-public-static-showcase/change-requests/cr-fps-004-dashboard-evidence-freshness.md) keeps static dashboard gate evidence current, [CR-FPS-005](./f-public-static-showcase/change-requests/cr-fps-005-deployment-catchup-proof.md) records deployed content-hash/manifest-contract parity after `main` deployment caught up, [CR-FPS-006](./f-public-static-showcase/change-requests/cr-fps-006-canonical-dashboard-source.md) replaces the inline dashboard fixture with a generated canonical local result-store artifact, and [CR-FPS-007](./f-public-static-showcase/change-requests/cr-fps-007-public-probe-parity-status.md) makes the standalone public probe fail closed while the deployed `dataHash` is stale.
...
- F public hosting proof: GitHub Pages is configured for `main` `/docs`; `https://austinyuch.github.io/finance_algorithms/` returned HTTP 200 and deployed `deployment-manifest.json` returned HTTP 200. After CR-FPS-006, the branch-local generated dashboard payload has a new `dataHash`, so current branch proof and the standalone public probe are intentionally `configured_not_observed` until deployment catch-up confirms the refreshed hash.
  - F browser visual proof: `docs/browser-visual.json` records Chromium screenshot hash `38f9d1e6cc5d7a3dc9e395db41ee5dc5c2464e618062cf305f1f473db49d333e`.
...
- **F browser pixel baseline lane:** [f-browser-pixel-baseline](./f-browser-pixel-baseline/) is **Implemented · Review PASSED**.
  ...
  - Latest browser visual diff: `1022 / 1,296,000` mismatched pixels, `mismatchRatio=0.0007885802469135803`, threshold `0.001`.

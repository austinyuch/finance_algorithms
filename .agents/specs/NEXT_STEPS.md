...
- **Current branch lane:** `spec/f-showcase-real-payload` (CR-FPS-006 canonical dashboard source overlay against completed F baseline).
...
- F public hosting proof: GitHub Pages is configured for `main` `/docs`; `https://austinyuch.github.io/finance_algorithms/` returned HTTP 200 and deployed `deployment-manifest.json` returned HTTP 200. After CR-FPS-006, the branch-local generated dashboard payload has a new `dataHash`, so current branch proof is intentionally `configured_not_observed` until deployment catch-up confirms the refreshed hash.
  - F browser visual proof: `docs/browser-visual.json` records Chromium screenshot hash `2bbe4348ca8c005bd6db5b472e7fa1c468fba1249c3288fe2319d1656131f493`.
...
- **F browser pixel baseline lane:** [f-browser-pixel-baseline](./f-browser-pixel-baseline/) is **Implemented · Review PASSED**.
  ...
  - Latest browser visual diff: `1019 / 1,296,000` mismatched pixels, `mismatchRatio=0.0007862654320987655`, threshold `0.001`.

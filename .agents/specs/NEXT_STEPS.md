...
- **Next gaps 1-6 Tier3/Public/Ops:** [next-gaps-1-6-tier3-public](./next-gaps-1-6-tier3-public/) is **Implemented · Review PASSED** in the working tree.
  - F public hosting proof: GitHub Pages is configured for `main` `/docs`; `https://austinyuch.github.io/finance_algorithms/` returned HTTP 200, deployed `deployment-manifest.json` returned HTTP 200, and `docs/deployment-manifest.json` records `hostingEvidence.status=proven` only with matching deployed `dataHash` and manifest contract metadata. Current branch-local static evidence refresh changes `dataHash`, so the committed manifest is `configured_not_observed` until deployment catches up.

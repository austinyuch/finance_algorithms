...
1. Use `scripts/snapshot_ops_gate.py --require-source-quorum` before claiming broad daily snapshot source readiness; scoped CR-B12 live-write smoke is not sufficient for that claim.
2. Keep `daily-snapshot.yml` observer evidence current after future cron runs; do not regress autonomous schedule proof from run `27392471359` back to pending without fresher contradictory Actions evidence.

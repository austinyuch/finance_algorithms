...
- INTEGRATION/SMOKE: `cd frontend && npm run build && npm run visual && npm run visual:browser && npm run smoke` -> passed; current browser visual diff is `0 / 1,296,000` under threshold `0.001`, and smoke uses a dynamically selected local port with occupied-`3044` chaos coverage.

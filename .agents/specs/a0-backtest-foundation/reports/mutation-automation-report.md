...
Current governance truth is no longer the initial A0-only runner size. The maintained suite is the cross-spec `scripts/run_mutation_spot_checks.py` registry referenced from `quantlab/TESTS.md`; current evidence is **106/106 configured/killed**, including CR-A0 event replay, `result-store-finite-oos-net-sharpe`, B source-quorum/Stooq proof gates, F public-hosting/stakeholder payload drift gates, F visual diff contract gating, F public probe freshness/observedAt timestamp and age gating, stale F Next.js fixture review wording, governance refresh review stale-evidence regression, E Tier3 production-readiness gates, Tier3 gate proof-digest guard, Tier3 gate production-validator guard, Tier3 manifest/experiment-binding/retraining artifact URI guards, production drift threshold guard, and governance stale-evidence guards.
...
- Mutation suite: **106/106 configured/killed** via `uv run python scripts/run_mutation_spot_checks.py`.

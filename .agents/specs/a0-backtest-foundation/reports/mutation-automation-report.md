...
Current governance truth is no longer the initial A0-only runner size. The maintained suite is the cross-spec `scripts/run_mutation_spot_checks.py` registry referenced from `quantlab/TESTS.md`; current evidence is **70/70 configured/killed**, including CR-A0 event replay, B source-quorum/Stooq proof gates, F public-hosting/stakeholder payload drift gates, E Tier3 production-readiness gates, Tier3 gate proof-digest guard, Tier3 gate production-validator guard, and governance stale-evidence guards.
...
- Mutation suite: **70/70 configured/killed** via `uv run python scripts/run_mutation_spot_checks.py`.

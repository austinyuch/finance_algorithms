...
4. **Mutation 自動化**:mutmut 3.x sandbox 與本 layout 不相容;repo-local `scripts/run_mutation_spot_checks.py` 已成為跨 spec governance runner,目前 100/100 configured mutations killed。完整 mutmut score 仍非本 slice 目標。
5. **import-linter / AST 雙層框架隔離**:框架隔離已由 `test_a0_0` AST 測試與 `uv run lint-imports` 契約共同守住;目前 import-linter KEPT over 75 files / 189 dependencies。

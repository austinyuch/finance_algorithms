...
> 驗證:`uv run pytest -q` → **66 passed**;`uv run mypy quantlab/ --ignore-missing-imports` → clean(24 檔);mutation spot-check **5/5 killed**。
...
4. **Mutation 自動化**:mutmut 3.x sandbox 與本 layout 不相容 → 改手動代表性變異 spot-check(5/5 killed)。自動 runner 重接列為待辦。

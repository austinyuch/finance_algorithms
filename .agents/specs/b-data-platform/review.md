...
- **Stooq / 個股(含 TSMC 2330):** 沙箱回 404,須在真機/有網路執行;`daily_snapshot.py` 的 `STOOQ_SYMBOLS` 已備清單。成功標準:`data/vintage/raw/<date>/stooq_*.json` 有非空 close。
...
- Stooq/個股 bulk fetch remains real-machine/source-contract work, and Stooq default source stays disabled until a working contract is selected.

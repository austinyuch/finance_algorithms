...
- **CR-B21 deep historical backfill → Implemented(repo-side + live run)**:`scripts/backfill_history.py` fetches deep history (Yahoo `period1=1990`, full FRED series, NOAA ONI) into `data/vintage/raw/backfill-1990-01-01/`, every record **`is_approximate=true`** + `backfill=true` (NOT true PIT — `available_date`=capture date; FRED macro latest-*revised*). Per-source degradation/retry/idempotent skip; `_backfill_manifest.json` records the run. Live run captured **18/24 sources** (all 11 deep Yahoo histories incl. `^GSPC` 9179 rows 1990→2026, 6 FRED, NOAA); 6 FRED rate/FX series `fail:ReadTimeout` under transient throttle and are an idempotently-completable residual. **Strict mode excludes the backfill** (the degenerate-flat-OOS CLI guard re-pinned to strict true-PIT still fails closed); `approximate_availability=True` yields a real multi-cycle comparison (deep {`^GSPC`,`^IXIC`} 1990→2026, BuyAndHold OOS-net Sharpe 0.7007 vs SmaTiming 0.2264, `no_alpha_claim`). Drawdowns validate against the real record (dot-com −49%/−78%, GFC −57%, COVID −34%, 2022 −25%).

## Residual / 刻意降級
- is_approximate lag 估算(無 vintage 源的 pre-collection 歷史)為前瞻能力,日常 snapshot 仍為 `is_approximate=false`;**CR-B21 的深度 1990+ backfill 即政策 Decision 3 的近似 bulk backfill**,以 `is_approximate=true` 落地(research-mode、strict 排除),非真實 vintage lag 表。
- 6 FRED rate/FX 系列(DGS10/DGS2/T10Y2Y/NASDAQCOM/DCOILWTICO/DEXTAUS)待 FRED throttle 解除後 idempotent 重跑 `scripts/backfill_history.py --since 1990-01-01` 補齊。

...
| **B-3** ◑ | 歷史 bulk backfill | CRAWL-001 | **FRED 全史已被 snapshot 涵蓋**;Stooq/個股=真機 handoff(見 review.md);**深度 1990+ 近似 backfill 已落地(CR-B21**:`scripts/backfill_history.py`,`is_approximate=true`、per-source degradation/idempotent、18/24 sources;strict 模式排除、`no_alpha_claim`);6 FRED rate/FX 系列已於 idempotent 重跑補齊(manifest `fail=0`,`T10Y2Y` 等可用,un-degrade CR-RDO-005 regime family) |

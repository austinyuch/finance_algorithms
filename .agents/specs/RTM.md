...
|---|---|---|---|---|
| A0 | Lookahead-safe vectorized/low-frequency event replay backtest with OOS-net metrics, walk-forward, parallel run, tracking | `quantlab/{contracts,data,engine,parallel,tracking}` | `test_a0_0..5`; CR-A0 regime scheduling; CR-A0 event replay; current mutation spot checks 63/63 configured/killed | **PASSED** |
...
| Gate | Command | Result |
|---|---|---|
| Python suite | `uv run pytest -q` | **247 passed** |
...
| Browser visual | `cd frontend && npm run visual:browser` | `proven`; repo-baseline pixel diff `1049 / 1,296,000` at threshold `0.001` |

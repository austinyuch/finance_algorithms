...
|---|---|---|---|---|
| A0 | Lookahead-safe vectorized/low-frequency event replay backtest with OOS-net metrics, walk-forward, parallel run, tracking | `quantlab/{contracts,data,engine,parallel,tracking}` | `test_a0_0..5`; CR-A0 regime scheduling; CR-A0 event replay; current mutation spot checks 62/62 configured/killed | **PASSED** |
...
| Python mutation | `uv run python scripts/run_mutation_spot_checks.py` | 62/62 configured mutations killed, including CR-A0 `engine-event-driven-date-gate`, CR-B12 `snapshot-scoped-source-health`, CR-B19 `b-source-quorum-proof-exit-gate` / `b-source-quorum-proof-file-gate`, CR-B20 `b-stooq-proof-exit-gate` / `b-stooq-proof-file-gate`, governance `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, `governance-stale-cron-proof-pending`, `governance-exhaustive-pr-ledger-regression`, `governance-stale-dashboard-source-wording`, CR-FPS-001 `public-hosting-manifest-status-overclaim`, CR-FPS-007 `public-hosting-probe-status-overclaim` / `review-public-hosting-probe-status-overclaim` / `public-hosting-probe-hash-overclaim`, CR-FPS-002 `public-hosting-manifest-hash-overclaim`, CR-FPS-003 `public-hosting-manifest-contract-regression`, `public-hosting-taxonomy-authority-regression`, `manual-showcase-payload-sync-regression`, `frontend-showcase-payload-sync-regression`, `review-pytest-gate-transcript-regression`, `review-frontend-count-shorthand-regression`, `review-audit-gate-transcript-regression`, `governance-test-registry-count-drift`, `mutation-test-registry-count-drift`, `governance-stale-dashboard-source-wording`, `f-showcase-retired-fixture-marker`, `governance-f-cr-superseded-fixture-boundary`, `public-hosting-probe-expected-hash-drift`, `governance-test-registry-count-drift`, and `mutation-test-registry-count-drift`; CR-FPS-008 is covered by the frontend mutation row |

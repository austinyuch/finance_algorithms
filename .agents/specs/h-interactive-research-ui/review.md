...
Public Pages parity is **not proven** for this branch artifact. The committed expected
`dataHash` is `c33da57d11c48945abcee36f2c78eb377f793536f769ddb10b87e8e4b3c7462a`,
while the live Pages probe still observes deployed `e57942605fd39fe21a5910164aad9e4af79d2d8aad9e28d7b04e858d01475953`.
Current public status is therefore `configured_not_observed` / `mismatched` until
deployment serves the refreshed artifact. The dashboard self-claim remains
`publicHosting=not_proven` by contract.
...
| REQ-H3-PUBLIC-001 public honesty | PASS with deploy-gated residual | Probe records `configured_not_observed`; no live/public execution or production Tier3 claim |
...
- Public probe: HTTP 200 plus manifest 200, but exits nonzero as expected because the
  deployed hash is stale versus expected `c33da57d…`.

## Residuals

- **Deploy-coupled public proof:** re-run the public probe after this branch artifact is
  deployed; public readiness may only become `proven` when deployed and expected hashes
  match with fresh evidence.

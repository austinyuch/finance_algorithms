...
- `uv run pytest -q tests/quantlab/test_e_1_experiment_registry.py tests/test_mutation_spot_checks.py` -> 38 passed.
- `uv run pytest --cov=quantlab.mlops.experiment_registry --cov-report=term-missing tests/quantlab/test_e_1_experiment_registry.py` -> 29 passed, 99% line coverage.
...
| REQ-E-PRODPROBE-004 | PASS | governed production triplet satisfies Tier3 gate only after validators accept traceable external proof URIs; malformed artifacts fail validators |

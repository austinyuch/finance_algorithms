...
|---|---|---|---|
| `a0-backtest-foundation` | `quantlab/TESTS.md` | A0 tests + CR-A0 regime scheduling + mutation automation | `uv run pytest -q` included in 190 passed; mypy clean(50 files); import-linter KEPT |
| `a-tsmc-hedge-slice` | `quantlab/TESTS.md` | 17 Epic A tests | `uv run pytest -q` included in 190 passed |
| `b-data-platform` / `b-snapshot-ops-gate` / `ops-visual-drift-artifacts` / `b-live-scheduled-snapshot-proof` | `quantlab/TESTS.md` | B/data tests including Yahoo fallback, Stooq opt-in, source-health registry, daily snapshot unit tests, CR-B11 run-report JSON, ops report validation, schedule report/proof retention, workflow contract, workflow timestamp guard, Actions `workflow_dispatch` proof, and Stooq source-contract reopen evidence helper | `uv run pytest -q` included in 190 passed; `tests/test_daily_snapshot.py` 24 passed; Actions run `27387041974` succeeded with artifact `snapshot-schedule-proof`; ops-visual-drift fallback trace coverage 100% for `source_health`, `snapshot_schedule_report`, `daily_snapshot`, and mutation runner; Python mutations killed |
| `c-portfolio-core` | `quantlab/TESTS.md` | 17 C tests including C-2 multi-horizon and C-3 rebalance coverage | `uv run pytest -q` included in 190 passed |

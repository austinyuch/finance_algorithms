...
| legacy `invest_algorithms` | `quantlab/TESTS.md` | 33 pyramid calculator regression tests | `uv run pytest -q` included in 193 passed, 1 skipped |
| governance guards | `quantlab/TESTS.md` | 5 import/drift/current-governance freshness guard tests | `uv run pytest -q` included in 193 passed, 1 skipped; `uv run lint-imports` KEPT; `governance-stale-next-steps-alert` and `governance-stale-scheduled-observer-promotion` mutations killed |

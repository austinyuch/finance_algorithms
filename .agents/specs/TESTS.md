...
| `c-portfolio-core` | `quantlab/TESTS.md` | 17 C tests including C-2 multi-horizon and C-3 rebalance coverage | `uv run pytest -q` included in 226 passed |
...
| legacy `invest_algorithms` | `quantlab/TESTS.md` | 33 pyramid calculator regression tests | `uv run pytest -q` included in 226 passed |
| governance guards | `quantlab/TESTS.md` | 13 import/drift/current-governance freshness guard tests, including CR-B12, CR-B13, CR-B14, CR-B15, CR-B16/CR-B17 post-promotion resume state, and CR-FPS-001 public-hosting manifest proof | `uv run pytest -q` included in 226 passed; targeted CR-FPS-001 and CR-B17 guards passed after memo/manifest regeneration; `scripts.run_mutation_spot_checks` focused coverage 90%; `uv run lint-imports` KEPT; `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, `governance-stale-cron-proof-pending`, `governance-stale-live-write-promotion`, `governance-stale-cr-b13-promotion`, `governance-stale-cr-b14-promotion`, `governance-stale-cr-b15-promotion`, `governance-stale-cr-b16-promotion`, `governance-stale-cr-b17-promotion`, and `public-hosting-manifest-status-regression` mutations killed |

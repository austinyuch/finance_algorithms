...
| `b-data-platform` / `b-snapshot-ops-gate` / `ops-visual-drift-artifacts` | `quantlab/TESTS.md` | B/data tests including Yahoo fallback, Stooq opt-in, source-health registry, daily snapshot unit tests, CR-B11 run-report JSON, ops report validation, schedule report/proof retention, workflow contract, and Stooq source-contract reopen evidence helper | `uv run pytest -q` included in 190 passed; ops-visual-drift fallback trace coverage 100% for `source_health`, `snapshot_schedule_report`, `daily_snapshot`, and mutation runner; Python mutations killed |

...
| `c-portfolio-core` | `quantlab/TESTS.md` | 17 C tests including C-2 multi-horizon and C-3 rebalance coverage | `uv run pytest -q` included in 238 passed |
...
| legacy `invest_algorithms` | `quantlab/TESTS.md` | 33 pyramid calculator regression tests | `uv run pytest -q` included in 238 passed |
| governance guards | `quantlab/TESTS.md` | 10 import/drift/current-governance freshness guard tests, including scheduled-observer, E gate, cron proof, non-self-staling promotion-boundary resume state, CR-FPS-001/CR-FPS-002/CR-FPS-003 public-hosting manifest/probe/hash/contract proof, and current visual evidence asset sync | `uv run pytest -q` included in 238 passed; targeted promotion-boundary, manifest, and visual evidence sync guards passed after memo regeneration; Stooq proof/source-health focused coverage 90%; `uv run lint-imports` KEPT; `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, `governance-stale-cron-proof-pending`, `governance-exhaustive-pr-ledger-regression`, `public-hosting-manifest-status-overclaim`, `public-hosting-manifest-hash-overclaim`, and `public-hosting-manifest-contract-regression` mutations killed |

...
| `e-mlops-tier3-lite` / `e-f-registry-dashboard-bridge` / `e-registry-durability-bridge` / `e-tier3-readiness-gate` / `e-tier3-serving-evidence` / `e-tier3-retraining-evidence` / `e-tier3-production-evidence-gate` / `next-gaps-1-6-tier3-public` / `ops-visual-drift-artifacts` | `quantlab/TESTS.md` | E-lite experiment lineage/config registry tests, checksum snapshot, result-store bridge, non-serving Tier3 artifact manifest, drift skeleton/assessment, fail-closed Tier3 readiness gate, local serving/retraining smoke evidence, production-tier evidence gate, local automated drift monitoring smoke evidence, and F dashboard registry read bridge | E targeted 24 passed; `e-tier3-readiness-gate`, `e-serving-smoke-health-gate`, `e-retraining-smoke-status-gate`, `e-tier3-production-tier-gate`, and `e-automated-drift-status-gate` mutations killed; focused `experiment_registry` line coverage 100% |
...
| legacy `invest_algorithms` | `quantlab/TESTS.md` | 33 pyramid calculator regression tests | `uv run pytest -q` included in 207 passed, 1 skipped |
| governance guards | `quantlab/TESTS.md` | 7 import/drift/current-governance freshness guard tests | `uv run pytest -q` included in 207 passed, 1 skipped; `uv run lint-imports` KEPT; `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, and `governance-stale-e-gate-promotion` mutations killed |

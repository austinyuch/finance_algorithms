...
| `c-portfolio-core` | `quantlab/TESTS.md` | 17 C tests including C-2 multi-horizon and C-3 rebalance coverage | `uv run pytest -q` included in 223 passed |
...
| legacy `invest_algorithms` | `quantlab/TESTS.md` | 33 pyramid calculator regression tests | `uv run pytest -q` included in 223 passed |
| governance guards | `quantlab/TESTS.md` | 10 import/drift/current-governance freshness guard tests, including CR-B12, CR-B13, CR-B14, and CR-B15 post-promotion resume state | `uv run pytest -q` included in 223 passed; targeted CR-B16 guard passed after memo sync; `scripts.run_mutation_spot_checks` focused coverage 90%; `uv run lint-imports` KEPT; `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, `governance-stale-cron-proof-pending`, `governance-stale-live-write-promotion`, `governance-stale-cr-b13-promotion`, `governance-stale-cr-b14-promotion`, and `governance-stale-cr-b15-promotion` mutations killed |

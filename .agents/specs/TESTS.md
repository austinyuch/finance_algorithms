...
| `c-portfolio-core` | `quantlab/TESTS.md` | 17 C tests including C-2 multi-horizon and C-3 rebalance coverage | `uv run pytest -q` included in 216 passed, 1 skipped |
...
| legacy `invest_algorithms` | `quantlab/TESTS.md` | 33 pyramid calculator regression tests | `uv run pytest -q` included in 216 passed, 1 skipped |
| governance guards | `quantlab/TESTS.md` | 7 import/drift/current-governance freshness guard tests | `uv run pytest -q` included in 216 passed, 1 skipped; `uv run lint-imports` KEPT; `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, and `governance-stale-cron-proof-pending` mutations killed |

...
| governance guards | `quantlab/TESTS.md` | 7 import/drift/current-governance freshness guard tests | `uv run pytest -q` included in 214 passed, 1 skipped; `uv run lint-imports` KEPT; `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, and `governance-stale-cron-proof-pending` mutations killed |

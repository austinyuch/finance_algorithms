...
| `f-showcase-read-api-dashboard` / `f-nextjs-showcase-dashboard` / `f-demo-hardening` / `f-public-demo-readiness` / `f-public-static-showcase` / `next-gaps-1-6-tier3-public` / `ops-visual-drift-artifacts` / `f-browser-pixel-baseline` | `quantlab/TESTS.md`; `frontend/tests/dashboard.test.tsx`; `frontend/tests/public-demo.test.tsx` | F Python read API plus real Next.js dashboard route/component tests, static public showcase export, hosted public Pages proof, deployment manifest/probe/content-hash/contract consistency guard, dashboard gate-evidence freshness guard, canonical local result-store payload source guard, browser visual evidence/diff, visual contract baseline, repo-baseline pixel diff gate, integration, and production HTTP smoke | Python F 6 passed; governance manifest/hash/contract, source-metadata, current visual evidence sync, and promotion-boundary guards included in 238 passed; Next.js F 27 passed, 91.07% line coverage, npm audit 0 vulnerabilities, 13 frontend mutations killed, visual/browser visual pixel diff/build/smoke/public probe passed |

...
| `c-portfolio-core` | `quantlab/TESTS.md` | 17 C tests including C-2 multi-horizon and C-3 rebalance coverage | `uv run pytest -q` included in 221 passed |
...
| legacy `invest_algorithms` | `quantlab/TESTS.md` | 33 pyramid calculator regression tests | `uv run pytest -q` included in 221 passed |
| governance guards | `quantlab/TESTS.md` | 8 import/drift/current-governance freshness guard tests, including CR-B12 post-promotion resume state | `uv run pytest -q` included in 221 passed; targeted CR-B13 guard passed after memo sync; `scripts.run_mutation_spot_checks` focused coverage 90%; `uv run lint-imports` KEPT; `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, `governance-stale-cron-proof-pending`, and `governance-stale-live-write-promotion` mutations killed |

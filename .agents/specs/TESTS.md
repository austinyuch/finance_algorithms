...
| governance guards | `quantlab/TESTS.md` | 10 import/drift/current-governance freshness guard tests, including CR-B12, CR-B13, and CR-B14 post-promotion resume state | `uv run pytest -q` included in 223 passed; targeted CR-B15 guard passed after memo sync; `scripts.run_mutation_spot_checks` focused coverage 90%; `uv run lint-imports` KEPT; `governance-stale-next-steps-alert`, `governance-stale-post-merge-sync-promotion`, `governance-stale-cron-proof-pending`, `governance-stale-live-write-promotion`, `governance-stale-cr-b13-promotion`, and `governance-stale-cr-b14-promotion` mutations killed |
